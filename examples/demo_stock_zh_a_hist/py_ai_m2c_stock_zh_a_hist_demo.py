"""
使用 qdata 库的核心功能生成代码
1. keyword 查询方式生成 stock_zh_a_hist_keyword.py
2. MCP 方式（AI tool calling）生成 stock_zh_a_hist_mcp.py
...
# 从 examples/demo_stock_zh_a_hist 到 AIQuant/QData
...
async def generate_mcp_code_async(markdown_content: str, output_file: str):
    ...
    # 如果参数中包含"20日"，在参数中添加标记以便代码生成时处理
    if '20日' in markdown_content or ('20' in markdown_content and '日' in markdown_content):
        if '_limit_days' not in params:
            params['_limit_days'] = '20日'  # 标记需要限制为20条记录
...
def main():
    ...
    # 读取 markdown 文件
    md_file = current_dir / "stock_zh_a_hist.md"
    ...
    # 生成 keyword 方式代码
    keyword_file = current_dir / "stock_zh_a_hist_keyword.py"
    generate_keyword_code(markdown_content, str(keyword_file))
    
    # 生成 MCP 方式代码
    mcp_file = current_dir / "stock_zh_a_hist_mcp.py"
